...
pub trait Timer {
    ...
    fn start(&self) -> impl Future<Output = Result<(), SchedulerError>>;
    /// Aborts the timer early
    fn stop(&self) -> impl Future<Output = Result<(), SchedulerError>>;
    /// Resets the timer by stopping and restarting it
    fn reset(&self) -> impl Future<Output = Result<(), SchedulerError>>;
